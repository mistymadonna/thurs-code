...
var Order  = function(customerName, text) {
...
Order.prototype.render = function() {
  ...
  liEl.innerHTML = '<img width="100px" src="img/' + this.customerName + '.jpg"> <b>' + this.customerName + ': </b><em>' + this.text + '</em>';
  return liEl;
};

var chatList = document.getElementById('customer-list');
var chatForm = document.getElementById('customer-form');
var clearChatList = document.getElementById('clear-customer-list');
var allOrders = [];
...
function doOrderSubmit(event) {
  console.log(event);
  event.preventDefault();

  if (!event.target.needs.value || !event.target.who.value) {Rimg
    return alert('You gotta put something here, mamma mia!');
  }

  var customer = event.target.who.value;
  var remark = event.target.needs.value;

  }

  var newOrder = new Order(customer, remark);

  console.log('Order from ' + event.target.who.value + ' at ' + Date());
  event.target.who.value = null;
  event.target.needs.value = null;

  allOrderss.push(newOrder);
  renderAllOrders();
};


orderForm.addEventListener('submit', doOrderSubmit);
});
